package com.miso.thegame.gameMechanics.objects.collisionHandlers;
...
public class Quadtree {
    ...
    private PositionIndexer getNonMovableObjectIndex(GameObject gameObject) {
        ...
        position.addToLeftSector = (gameObject.x <= midpointXaxis);
        position.addToRightSector = (gameObject.x + gameObject.getImage().getWidth() >= midpointXaxis);
        position.addToTopSector = (gameObject.y <= midpointYaxis);
        position.addToBottomSector = (gameObject.y + gameObject.getImage().getHeight() >= midpointYaxis);
    ...
    private PositionIndexer getMovableObjectIndex(GameObject gameObject) {
        ...
        PositionIndexer position = new PositionIndexer();
        position.addToLeftSector = (gameObject.x - gameObject.getHalfWidth() / 2 <= midpointXaxis);
        position.addToRightSector = (gameObject.x + gameObject.getHalfWidth() / 2 >= midpointXaxis);
        position.addToTopSector = (gameObject.y - gameObject.getHalfHeight() / 2 <= midpointYaxis);
        position.addToBottomSector = (gameObject.y + gameObject.getHalfHeight() / 2 >= midpointYaxis);
